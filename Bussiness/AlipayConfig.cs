...
namespace Com.Alipay
{
    ...
    internal class Config
    {
        ...
        static Config()
        {
            ...
            partner = "2088701020787394";

            //交易安全检验码，由数字和字母组成的32位字符串
            key = "gubdqhtq0a2fjzvo3x7d8wwa8d9gzjvd";

            //签约支付宝账号或卖家支付宝帐户
            seller_email = "niuzei2011@163.com";

            string sRootUrl = ProvideCommon.GetRootURI();

            //页面跳转同步返回页面文件路径 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
            return_url = string.Format("{0}/Pay/PayAli_Return.aspx",sRootUrl);

            //服务器通知的页面文件路径 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
            notify_url = string.Format("{0}/Pay/PayAli_Notify.aspx",sRootUrl);
