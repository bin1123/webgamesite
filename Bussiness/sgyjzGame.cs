   ...
   public class sgyjzGame
    {
        public static string Login(string sUserID, string sGame,string fcm)
        {
            ...
            string key = "26dee06f1d7e4fe38a4e5797c603647c";
            ...
            sbText.AppendFormat("http://s{0}.sgyjz.dao50.com/index.php?", serverid);
            sbText.AppendFormat("accountId={0}", sUserID);
            sbText.AppendFormat("&accountName={0}", sUserID);
        ...
        {
            decimal dJiaoMoney = dMoney * 10;//角
            int cash = Convert.ToInt32(dJiaoMoney);//角
            int iGameMoney = cash;//金币
            string serverid = GetServerID(sGame);
            string ts = ProvideCommon.getHMTime();
            string key = "48f0d8465fda447a8756751ff2b08356";
            StringBuilder sbText = new StringBuilder();
            ...
            sbText.Remove(0, sbText.Length);
            string sGamePayUrl = string.Format("http://s{0}.sgyjz.dao50.com:9001/recharge",serverid);
            sbText.AppendFormat("cash={0}", cash.ToString());
            sbText.AppendFormat("&money={0}", iGameMoney.ToString());
            sbText.AppendFormat("&orderId={0}", sOrderID);
            sbText.AppendFormat("&ts={0}", ts);
        ...
        public static string sgyjzPay(string sGameAbbre, int iUserID, string sUserName, int iPayPoints, string sPhone, int iGUserID)
        {
        ...
        public static string sgyjzQucikPay(string sGameAbbre, string sUserName, decimal dPrice, string sTranID)
        {
        ...
        {
            string sID = sGame.Replace("sgyjz", "");
            return sID.ToString();
        }
        ...
        public static string GameisLogin(string sUserID, string sGameAbbre)
        {
            ...
            string ts = ProvideCommon.getHMTime();
            string key = "d3230183d9c2477692d01c0580a10950";
            StringBuilder sbText = new StringBuilder();
            sbText.AppendFormat("account={0}", sUserID);
            ...
            sbText.Remove(0, sbText.Length);
            sbText.AppendFormat("http://s{0}.sgyjz.dao50.com:9001/checkuser?", serverid);
            sbText.AppendFormat("account={0}", sUserID);
        ...
        public static string GetNewCode(string sUserID,string sGameAbbre,string sCodeType)
        {
            ...
            StringBuilder sbText = new StringBuilder();
            sbText.AppendFormat("account={0}", sUserID);
            string couponId = "10005";//兑换码的礼包ID
