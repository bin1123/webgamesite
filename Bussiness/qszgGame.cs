...
namespace Bussiness
{
    public class qszgGame
    {
        private const string key = "jqWvBBBKFxbqmS4z";
        ...
        public static string Login(string sUserID, string sGame)
        {
            ...
            string sAuth = string.Format("op_id={0}&sid={1}&game_id=36&account={2}&adult_flag=1&game_time=&ip=&ad_info={3}&time={4}",op_id,sid,account,ad_info,time);
        ...
        public static string qszgPay(string sGameAbbre, int iUserID, string sUserName, int iPayPoints, string sPhone, int iGUserID)
        {
        ...
        public static string qszgQucikPay(string sGameAbbre, string sUserName, decimal dPrice, string sTranID)
        {
        ...
        {
            string sServerID = sGame.Replace("qszg", "");
            int iBeginServerID = 50000000;
            int iSID = 1;
            int.TryParse(sServerID, out iSID);
            int iServerID = iBeginServerID + iSID;
            string sGameID = string.Format("360011{0}", iServerID.ToString());
            return sGameID;
        }
    }
}
