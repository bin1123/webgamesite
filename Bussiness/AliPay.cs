...
namespace Bussiness
{
    ...
    public class AliPay
    {
        public static string PayDirect(string sChannel, string sPhone, string sAccount, decimal dPrice, int iCount,string sBankName)
        {
            ...
            string out_trade_no = TransPBLL.PointSalesInit(sChannel, sPhone, sAccount, dPrice, iCount,sTranIP);   //定单号;
            string body = string.Format("Dao50{0}元充值", dPrice.ToString());            //订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
            string extra_common_param = sAccount;   //自定义参数，可存放任何内容（除=、&等特殊字符外），不会显示在页面上
        ...
        { 
            string body = string.Format("Dao50{0}元游戏充值", dPrice.ToString());
